import { computeStats, formatNumber, formatWon } from "../lib/format.js";
import MonthlyGrid from "./MonthlyGrid.jsx";
import ItemCard from "./ItemCard.jsx";

export default function TotalSummaryCard({ group }) {
  const { total, totalRev, maxSale, maxRev } = computeStats(
    group.totalSummary.sales,
    group.totalSummary.revenue
  );

  return (
    <div className="item-group">
      <div className="result-card total-summary-card">
        <div className="card-header-flex">
          <h3 className="card-title">전체 브랜드 통합 출고 합계</h3>
          <span className="category-badge">{group.category || "-"}</span>
        </div>
        <div className="card-info-grid">
          <div className="info-box">
            <span className="label">품목코드</span>
            <span className="value">{group.itemCode || "-"}</span>
          </div>
          <div className="info-box">
            <span className="label">품목명</span>
            <span className="value">{group.itemName || "알수없음"}</span>
          </div>
        </div>

        <div className="stats-grid-inner">
          <div className="stats-box highlight">
            <span className="label">누적 판매량</span>
            <span className="value">{formatNumber(total)}</span>
          </div>
          <div className="stats-box highlight-rev">
            <span className="label">누적 매출액</span>
            <span className="value small">{formatWon(totalRev)}</span>
          </div>
          <div className="stats-box">
            <span className="label">월 최대 판매</span>
            <span className="value">{formatNumber(maxSale)}</span>
          </div>
          <div className="stats-box">
            <span className="label">월 최대 매출</span>
            <span className="value small">{formatWon(maxRev)}</span>
          </div>
        </div>

        <MonthlyGrid
          sales={group.totalSummary.sales}
          revenue={group.totalSummary.revenue}
          fitToScreen={true}
        />
      </div>

      <div className="brand-results-grid standalone">
        {group.brandResults.map((br) => (
          <ItemCard key={`${br.brand}-${br.itemCode}`} item={br} brand={br.brand} />
        ))}
      </div>
    </div>
  );
}
